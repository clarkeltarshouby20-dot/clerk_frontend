// English locale strings — used as the default fallback
export default {
  // ── Navigation ────────────────────────────────────────────
  nav: {
    home: "Home",
    products: "Products",
    cart: "Cart",
    orders: "My Orders",
    login: "Login",
    register: "Register",
    logout: "Logout",
    admin: "Admin Panel",
    dashboard: "Dashboard",
    profile: "Profile",
    wishlist: "Wishlist",
  },
 
  // ── Footer ────────────────────────────────────────────────
  footer: {
    rights: "All rights reserved.",
    builtWith: "Built with",
    tagline: "Experience the future of online shopping with modern touches and seamless interaction.",
    quickLinks: "Quick Links",
    contactInfo: "Contact Info",
    followUs: "Follow Us",
  },

  "JPEG, PNG, WEBP. Max 5MB.": "JPEG, PNG, WEBP. Max 5MB.",
  "Invalid file type.": "Invalid file type.",
  "File too large. Max size is": "File too large. Max size is",
  // ── Common / Shared ───────────────────────────────────────
  common: {
    login: "Login",
    loading: "Loading…",
    error: "Something went wrong. Please try again.",
    save: "Save",
    cancel: "Cancel",
    delete: "Delete",
    edit: "Edit",
    add: "Add",
    create: "Create",
    update: "Update",
    search: "Search",
    filter: "Filter",
    apply: "Apply",
    reset: "Reset",
    confirm: "Confirm",
    reject: "Reject",
    back: "Back",
    next: "Next",
    previous: "Previous",
    close: "Close",
    yes: "Yes",
    no: "No",
    optional: "optional",
    or: "or",
    submit: "Submit",
    viewAll: "View All",
    loadMore: "Load More",
    noResults: "No results found.",
    price: "Price",
    priceRange: "Price Range",
    category: "Category",
    all: "All",
    status: "Status",
    actions: "Actions",
    total: "Total",
    quantity: "Quantity",
    stock: "Stock",
    image: "Image",
    name: "Name",
    description: "Description",
    createdAt: "Created At",
    darkMode: "Dark Mode",
    lightMode: "Light Mode",
    language: "Language",
    action: "Action",
    view: "View",
    noData: "No data available.",
    product: "Product",
    contactUs: "Contact Us",
    copied: "Copied to clipboard!",
    typeSomething: "Type here...",
    change: "Change",
    remove: "Remove",
    dropNow: "Drop Now",
    uploadImage: "Upload Image",
    deleteConfirmTitle: "Confirm Deletion",
    deleteConfirmMsg: "Are you sure you want to delete this item permanently? This action cannot be undone.",
  },

  // ── Auth ──────────────────────────────────────────────────
  auth: {
    login: "Login",
    register: "Register",
    email: "Email",
    password: "Password",
    username: "Username",
    confirmPassword: "Confirm Password",
    forgotPassword: "Forgot password?",
    noAccount: "Don't have an account?",
    hasAccount: "Already have an account?",
    loginSuccess: "Welcome back!",
    registerSuccess: "Account created successfully!",
    logoutSuccess: "Logged out successfully.",
    invalidCredentials: "Invalid email or password.",
    emailExists: "This email is already registered.",
    phoneExists: "This phone number is already registered.",
    passwordUppercase: "Password must contain at least one uppercase letter.",
    passwordNumber: "Password must contain at least one number.",
  },

  // ── Home ──────────────────────────────────────────────────
  home: {
    heroTitle: "Discover Amazing Products",
    heroSubtitle: "Shop the latest trends at unbeatable prices.",
    shopNow: "Shop Now",
    categories: "Shop by Category",
    featuredProducts: "Featured Products",
    allProducts: "All Products",
    newArrivals: "New Arrivals",
    premiumWatches: "Premium Watches",
    bestDeals: "Best Deals",
    techAccessories: "Tech Accessories",
    newCollection: "Quiet Luxury Realized.",
    heroDescription: "Discover a curated selection of excellence. Explore our exclusive new arrivals designed for modern, effortless living.",
    heroTitlePart1: "Not for Everyone",
    heroTitlePart2: "Welcome to the Upper Class",
    exploreMore: "Explore More",
    topRated: "Top Rated",
    smartGear: "Smart Gear",
    mostVisited: "Most Visited",
    popularCategory: "Popular Category",
  },

  // ── Products ──────────────────────────────────────────────
  products: {
    title: "Products",
    addToCart: "Add to Cart",
    showingItems: "Showing {n} of {total} Items",
    outOfStock: "Out of Stock",
    inStock: "In Stock",
    details: "Product Details",
    gallery: "Gallery",
    relatedProducts: "Related Products",
    minPrice: "Min Price",
    maxPrice: "Max Price",
    sortBy: "Sort By",
    filterByCategory: "Filter by Category",
    searchPlaceholder: "Search products…",
    addSubtitle: "Create New Catalog Item",
    noProducts: "No products found.",
    addedToCart: "Added to cart!",
    choosePurchaseOptions: "Please choose the purchase options first.",
    items: "items",
    item: "item",
    lowStockBadge: "Only {n} left!",
    lowStock: "Low Stock",
    specs: "Specifications",
    reviews: "Reviews",
    reviewCount: "{n} review | {n} reviews",
    noReviews: "No reviews yet. Be the first to review this product!",
    writeReview: "Write a Review",
    yourRating: "Your Rating",
    yourComment: "Your Comment (optional)",
    submitReview: "Submit Review",
    reviewPublished: "Thank you! Your review has been published.",
    alreadyReviewed: "You have already reviewed this product.",
    loginToReview: "Please log in to leave a review.",
    suggestedForYou: "You May Also Like",
    basedOnRatings: "based on {n} reviews",
    verifiedPurchase: "Verified Purchase",
  },

  // ── Cart ──────────────────────────────────────────────────
  cart: {
    title: "Your Cart",
    empty: "Your cart is empty.",
    continueShopping: "Continue Shopping",
    checkout: "Proceed to Checkout",
    removeItem: "Remove",
    subtotal: "Subtotal",
    total: "Total",
    orderSummary: "Order Summary",
    shipping: "Shipping",
    free: "Free",
    removeItem: "Remove Item",
    quantity: "Quantity",
    emptySubtitle: "Looks like you haven't added anything yet.",
    itemsCount: "{count} item | {count} items",
    confirmRemove: "Remove this item from cart?",
  },

  // ── Checkout ─────────────────────────────────────────────
  checkout: {
    title: "Secure Checkout",
    emptyCart: "Your shopping cart is currently empty.",
    shippingInfo: "Shipping Information",
    detailedAddress: "Detailed Home Address",
    addressPlaceholder:
      "Street Name, Building Number, Apartment, City, Governorate",
    invalidPhoneEgypt:
      "Please enter a valid Egyptian mobile number (11 digits starting with 01).",
    orderSummary: "Order Summary",
    shippingFee: "Shipping Fee",
    paymentMethod: "Payment Method",
    cashOnDelivery: "Cash on Delivery",
    electronicWallet: "Electronic Wallet",
    instapay: "InstaPay",
    walletInstructionsTitle: "Transfer via Vodafone/Orange Cash to:",
    instapayInstructionsTitle: "Transfer via InstaPay to:",
    uploadProofRule:
      "You must send the shipping fee or the total to confirm your order",
    placeOrder: "Place Order",
    confirmOrder: "Confirm & Pay",
    orderPlaced: "Order placed successfully! Redirecting...",
    orderError: "An error occurred while placing the order.",
    paymentMethod: "Payment Method",
    electronicWallet: "Electronic Wallet",
    cashOnDelivery: "Cash on Delivery",
    fullName: "Full Name",
    phone: "Phone Number",
    codLabel: "Cash on Delivery",
    walletLabel: "Wallet",
    qty: "Qty",
    free: "Free",
    secureProcessing: "Your data is securely processed",
    fast: "Fast",
    orderSummary: "Order Summary",
    shippingFee: "Shipping Fee",
    securePayment: "Secure Payment",
    haveCoupon: "Have a discount code?",
    couponApplied: "Coupon Applied Successfully!",
    discount: "Discount",
    governorate: "Governorate",
    governorateSearchPlaceholder: "Search by governorate name",
    noGovernoratesFound: "No governorates match your search.",
    selectGovernorate: "Select",
    selectedGovernorate: "Selected",
    selectedGovernorateLabel: "Selected Governorate",
    clearGovernorate: "Clear selected governorate",
  },

  // ── Payment ──────────────────────────────────────────────
  payment: {
    title: "Upload Payment Proof",
    amount: "Amount Transferred",
    reference: "Reference Number",
    screenshot: "Transfer Screenshot",
    screenshotHint:
      "Upload a screenshot of your bank/wallet transfer (JPG, PNG, WEBP – max 5 MB)",
    submit: "Submit Proof",
    success: "Payment proof submitted! Awaiting admin verification.",
    pending: "Pending Verification",
    pendingDesc:
      "Your payment is under review. We'll update your order status shortly.",
    verified: "Payment Verified",
    rejected: "Payment Rejected",
  },

  // ── Orders ───────────────────────────────────────────────
  orders: {
    title: "My Orders",
    noOrders: "You have no orders yet.",
    orderId: "Order #",
    orderDate: "Date",
    orderStatus: "Order Status",
    paymentStatus: "Payment Status",
    orderItems: "Items",
    viewDetails: "View Details",
    pay: "Pay Now",
    trackOrder: "Track Order",
    trackTitle: "Track Order",
    backToOrders: "Back to Orders",
    tracker: {
      orderDate: "Order Date",
      paymentMethod: "Payment Method",
      totalAmount: "Amount",
      pricingBreakdown: "Pricing Breakdown",
      orderRejected: "Order Rejected",
      orderCancelled: "Order Cancelled",
      orderReturned: "Order Returned",
      rejectedDesc:
        "Your order has been cancelled due to payment rejection by the administration team.",
      returnedDesc:
        "This order was returned after delivery. Inventory was restored and it is excluded from net revenue.",
      rejectionReason: "Reason for Rejection",
      returnReason: "Reason for Return",
      liveStatus: "Live Tracking Status",
      currentStage: "Current Stage",
      includedItems: "Included Items",
      qty: "Qty",
      paymentReceipt: "Payment Receipt",
    },
    status: {
      pending: "Pending",
      confirmed: "Confirmed",
      verified: "Verified",
      shipped: "Shipped",
      out_for_delivery: "Out for Delivery",
      delivered: "Delivered",
      returned: "Returned",
      completed: "Completed",
      cancelled: "Cancelled",
      rejected: "Rejected",
      problem: "Problem",
    },
    paymentStatusLabels: {
      unpaid: "Unpaid",
      pending_verification: "Pending Verification",
      paid: "Paid",
      rejected: "Rejected",
    },
    timeline: {
      stepPending: "Order Placed",
      stepPaid: "Payment Verified",
      stepDelivered: "Delivered",
      pending: "Pending",
      confirmed: "Confirmed",
      outForDelivery: "Out for Delivery",
      delivered: "Delivered",
    },
  },

  // ── Wishlist ────────────────────────────────────────────────
  wishlist: {
    title: "My Wishlist",
    empty: "Your wishlist is empty.",
    added: "Added to wishlist",
    removed: "Removed from wishlist",
    shopNow: "Discover Products",
  },

  // ── Admin ─────────────────────────────────────────────────
  admin: {
    filters: {
      allTime: "All Time",
      byDay: "Day",
      byWeek: "Week",
      byMonth: "Month",
      byYear: "Year",
      panelTitle: "Filter Analytics",
      allTimeDescription: "Showing all dashboard data without a time restriction.",
      selectedRange: "Selected Range",
      chooseRangePrompt: "Choose a precise time range to refresh the dashboard.",
      apply: "Refresh Results",
      reset: "Reset",
      dayLabel: "Select Day",
      weekLabel: "Select Week",
      monthLabel: "Select Month",
      yearLabel: "Select Year",
    },
    user: "User",
    dashboard: "Dashboard",
    products: "Products",
    categories: "Categories",
    orders: "Orders",
    payments: "Payment Verification",
    reviewManagement: "Ratings & Reviews",
    deleteReview: "Delete Review",
    confirmDeleteReview:
      "Are you sure you want to delete this review? This action cannot be undone.",
    pending: "Pending",
    approved: "Public",
    hidden: "Hidden",
    comment: "Comment",
    status: "Status",
    toggleVisibility: "Toggle Visibility",
    settings: "Site Settings",
    users: "Users",
    totalOrders: "Orders",
    totalPosSales: "POS Sales",
    totalRevenue: "Revenue",
    totalNetProfit: "Net Profit",
    totalReturns: "Returns",
    totalExpenses: "Expenses",
    pendingPayments: "Pending Payments",
    totalProducts: "Products",
    totalUsers: "Users",
    verifyApprove: "Verify & Approve",
    rejectOrder: "Reject Order",
    verifyPayment: "Verify Payment",
    verifyPaymentMessage: "Are you sure you want to approve this payment and verify the order?",
    customerInfo: "Customer Info",
    purchasedItems: "Purchased Items",
    controlCenter: "Control Center",
    orderActions: "Order Actions",
    statusUpdated: "Order status updated successfully.",
    markShipped: "Mark as Shipped",
    markDelivered: "Mark as Delivered",
    returnProduct: "Return Order",
    transactionProof: "Transaction Proof",
    refId: "Reference ID",
    addProduct: "Add Product",
    editProduct: "Edit Product",
    deleteProduct: "Delete Product",
    addCategory: "Add Category",
    editCategory: "Edit Category",
    deleteCategory: "Delete Category",
    confirmDelete: "Are you sure? This will delete all products and images inside this category!",
    categoryDiscountBtn: "Category discount",
    categoryDiscount: {
      title: "Discount all products in category",
      apply: "Apply discount",
      remove: "Remove discount",
      hint: "Applies to the current selling price. The discount amount is added to each product's discount field and net profit is recalculated automatically.",
      typePercent: "Percentage %",
      typeFixed: "Fixed amount",
      valueLabel: "Discount value",
      valuePlaceholderPercent: "e.g. 20",
      valuePlaceholderFixed: "e.g. 150",
      success: "Discount applied to {count} product(s)",
      removeSuccess: "Discount removed from {count} product(s)",
      failed: "Failed to apply discount",
      invalidValue: "Enter a valid discount value",
    },
    expenses: {
      title: "Expenses",
      summaryTotal: "Expenses",
      amount: "Amount",
      amountPlaceholder: "0.00",
      add: "Add",
      date: "Date",
      empty: "No expenses recorded yet.",
      editTitle: "Edit expense",
      deleteTitle: "Delete expense",
      deleteConfirm: "Are you sure you want to delete this expense?",
      addSuccess: "Expense added",
      updateSuccess: "Expense updated",
      deleteSuccess: "Expense deleted",
      invalidAmount: "Enter a valid amount greater than zero",
    },
    confirmDeleteProduct: "Are you sure you want to delete this product? This action cannot be undone.",
    approve: "Approve",
    productImages: "Product Images",
    uploadImages: "Upload Images (max 10)",
    arabicName: "Arabic Name",
    slug: "Slug",
    parentCategory: "Parent Category",
    sortOrder: "Sort Order",
    isActive: "Active",
    statusActive: "Active",
    statusHidden: "Hidden",
    recentOrders: "Recent Orders",
    viewDetails: "View details",
    verifySuccess: "Payment verified — order marked as completed.",
    ownerOnly: "Owner Role Required",
    ownerOnlyDesc: "You can view settings but only the Owner can modify them.",
    users: {
      title: "Users Management",
      subtitle: "Manage users and change roles (Owner only)",
      countLabel: "users",
      empty: "No users found.",
      name: "Name",
      email: "Email",
      role: "Role",
      joined: "Joined",
      editTitle: "Edit User",
      roleWarning:
        "Changing a user to Admin or Owner grants them access to the dashboard.",
      ownerOnly: "Owner only",
      roles: {
        user: "User",
        admin: "Admin",
        owner: "Owner",
      },
      updateSuccess: "User updated successfully.",
      deleteTitle: "Delete User",
      deleteConfirm: "Are you sure you want to delete user {name}?",
      deleteSuccess: "User deleted successfully.",
      changeAvatar: "Change profile picture",
      avatarHint: "JPG, PNG or WebP. Max 5 MB.",
      newPassword: "New Password",
      passwordPlaceholder: "Leave empty to keep current password",
      passwordTooShort: "Password must be at least 8 characters.",
      passwordOk: "Password looks good ✓",
    },
    roleBadge: { admin: "Admin", owner: "Owner", user: "User" },
    messages: "Messages",
    ordersPipelineHint: "Drag orders between columns to update status",
    emptyColumn: "Empty",
    descEn: "Description (EN)",
    descAr: "Description (AR)",
    kanban: {
      columns: {
        new: "New / Pending",
        verified: "Verified",
        shipped: "Shipped",
        completed: "Completed",
        returned: "Returned",
      },
    },
    selectCategory: "Select Category...",
    imageRequirements: "JPG, PNG or WebP — max 5 MB each",
    imageRequirements2MB: "JPG, PNG or WebP — max 2 MB",
    saveError: "Error saving product. Please check the form.",
    currentPrice: "Current Price",
    originalPrice: "Original Price",
    sellingPrice: "Selling Price",
    discount: "Discount (Optional)",
    netProfit: "Net Profit / Unit",
    netProfitHint: "Calculated automatically: selling price − original price",
    oldPrice: "Original Price (Optional)",
    mainImage: "Main Image",
    productDetails: "Product Details",
    productDescription: "Product Description",
    gallery: "Gallery",
    mandatory: "Mandatory",
    pos: {
      title: "Point of Sale",
      sell: "Sell",
      history: "History",
      barcode: "Barcode",
      printBarcode: "Print Barcode",
      print: "Print",
      labelSizeHint: "Label size: {size} mm — matches standard thermal sticker rolls.",
      labelSetupTitle: "First-time printer setup",
      labelSetupStep1: "Install the label printer driver on Windows.",
      labelSetupStep2: "In Chrome print dialog: select your label printer (not Save as PDF).",
      labelSetupStep3: "Set Margins to None and turn off Headers and footers.",
      labelSetupStep4: "Set Paper size to {size} mm in the printer driver, then print.",
      scannerReady: "Scan Barcode",
      scannerHint: "Scan a product barcode or type it below, then press Enter.",
      scannerPlaceholder: "Product barcode…",
      scannerEnterHint: "Enter = find product and add to invoice",
      scannerActive: "Ready to scan",
      scannerPaused: "Scanning paused",
      cart: "Invoice",
      emptyCart: "Invoice is empty. Scan a product barcode to start.",
      item: "Product",
      variant: "Options",
      qty: "Qty",
      original: "Unit Price",
      discount: "Item Discount",
      final: "Price After Discount",
      total: "Line Total",
      remove: "Remove",
      totals: "Summary",
      subtotalBefore: "Before Discount",
      itemsDiscount: "Item Discounts",
      itemsSubtotal: "After Item Discounts",
      cartDiscount: "Invoice Discount",
      cartDiscountAmount: "Discount Amount",
      finalTotal: "Amount Due",
      itemsCount: "Items",
      fixed: "Fixed",
      clear: "Clear",
      color: "Color",
      size: "Size",
      quantity: "Quantity",
      stock: "Stock",
      itemDiscount: "Item Discount",
      finalPrice: "Price After Discount",
      discountBtn: "Discount",
      add: "Add to Invoice",
      sellBtn: "Complete Sale",
      returnBtn: "Return",
      cancelBtn: "Clear Invoice",
      processing: "Processing…",
      saleComplete: "Sale Complete",
      returnComplete: "Return Complete",
      returnFullInvoice: "Full invoice return",
      returnPartialInvoice: "Returning {count} item(s)",
      customizeReturn: "Choose items to return",
      doneCustomizeReturn: "Done — show summary",
      noReturnItemsSelected: "No items selected. Tap the pencil icon to choose items.",
      receiptNo: "Receipt No.",
      date: "Date",
      printReceipt: "Print Receipt",
      returnTitle: "Return Items",
      receiptPlaceholder: "e.g. POS-20260704-0001",
      search: "Search",
      returnable: "Remaining",
      processReturn: "Process Return",
      searchReceipt: "Search by receipt no.",
      dateFrom: "From",
      dateTo: "To",
      loading: "Loading…",
      noSales: "No sales or returns found.",
      type: "Type",
      cashier: "Cashier",
      view: "View / Print",
      productAdded: "\"{name}\" added to invoice",
      outOfStock: "\"{name}\" is out of stock",
      productNotFound: "No product found for this barcode",
      confirmSellTitle: "Complete Sale",
      confirmSellMessage: "Amount due: {total} — confirm cash payment?",
      saleCompleted: "Sale completed successfully",
      saleFailed: "Sale failed",
      confirmCancelTitle: "Clear Invoice",
      confirmCancelMessage: "Remove all items from the current invoice?",
      returnSaleOnly: "Returns are only allowed against sale receipts",
      receiptNotFound: "Receipt not found",
      returnCompleted: "Return processed successfully",
      returnFailed: "Return failed",
      receiptPrintHint: "80mm receipt — select your thermal receipt printer, turn off headers/footers, set margins to None.",
      receiptPrintFailed: "Unable to open print dialog. Check printer settings.",
    },
    coupons: {
      title: "Coupon Management",
      subtitle: "Create and manage discount codes for your customers.",
      add: "Create Coupon",
      empty: "No coupons found. Create your first discount code!",
      addTitle: "New Coupon",
      editTitle: "Edit Coupon",
      labelCode: "Coupon Code",
      labelType: "Discount Type",
      labelValue: "Discount Value",
      labelMinAmount: "Min. Order Amount",
      labelMaxUses: "Max Uses",
      labelExpiry: "Expiry Date",
      labelActive: "Coupon is Active",
      percentage: "Percentage (%)",
      fixed: "Fixed Amount",
      discount: "Discount Value",
      minOrder: "Min. Order",
      expiry: "Expires On",
      usage: "Usage Limit",
      deleteTitle: "Delete Coupon",
      deleteConfirm: "Are you sure you want to delete this coupon? This action cannot be undone.",
    },
    financeControl: "Finance Control",
    walletInstaPay: "Wallet & InstaPay Verification",
    pendingPayments: "pending payments",
    noProofs: "No pending payment proofs found.",
    viewScreenshot: "View Screenshot",
    approve: "Approve Payment",
    verifySuccess: "Payment verified — order marked as completed.",
    rejectReasonPrompt: "Please enter a reason for rejection (optional):",
    payments: "Payments",
    paymentsTable: {
      txn: "Txn #",
      order: "Order #",
      user: "User",
      amount: "Amount",
      orderTotal: "Order Amount",
      reference: "Reference",
      screenshot: "Screenshot",
      submitted: "Submitted",
    },
  },

  // ── Site Settings ─────────────────────────────────────────
  settings: {
    title: "Site Settings",
    saveSuccess: "Settings saved successfully!",
    saveError: "Failed to save settings. Please try again.",
    lastUpdated: "Last updated",

    // Section headings
    sectionIdentity: "Identity",
    sectionContact: "Contact",
      sectionSocial: "Social Links",
      sectionAds: "Advertising & Analytics",
      sectionScripts: "Custom Scripts",
      sectionShipping: "Shipping",

    // Identity fields
    siteName: "Site Name",
    siteNamePlaceholder: "e.g. My Awesome Store",
    logoUrl: "Site Logo",
    uploadLogo: "Upload Logo",
    faviconUrl: "Site Favicon",
    uploadFavicon: "Upload Favicon",
    logoHint: "JPG, PNG, WebP or SVG. Max 2 MB.",
    faviconHint: "PNG, ICO or SVG. Max 512 KB.",
    logoPreview: "Current Logo",
    logoError: "Invalid image file. Please try another.",
    noLogo: "No logo set yet.",

    // Contact fields
    contactEmail: "Contact Email",
    contactEmailPlaceholder: "support_at_mystore.com",
    whatsappNumber: "WhatsApp Number",
    whatsappPlaceholder: "+201012345678",
    currencyCode: "Currency Code (ISO 4217)",
    currencyPlaceholder: "EGP",

    // Advertising fields
    googleAnalyticsId: "Google Analytics ID",
    gAnalyticsPlaceholder: "G-XXXXXXXXXX",
    googleAdsClientId: "Google Ads Client ID",
    gAdsPlaceholder: "ca-pub-XXXXXXXXXXXXXXXX",

    // Script fields
    headerScripts: "Header Scripts",
    headerScriptsHint:
      "Injected into the page head. Use for Google Tag Manager, GA snippet, etc.",
    footerScripts: "Footer Scripts",
    footerScriptsHint:
      "Injected before the closing body tag. Use for Google Ads conversion tracking.",
    scriptsWarning:
      "Only add trusted scripts (Google, Meta, etc.). Malicious scripts can compromise your site.",

      sectionPayment: "Payment Methods Config",
      shippingGovernoratesTitle: "Egypt Governorates Delivery Pricing",
      shippingGovernoratesHint:
        "Enable the governorates you ship to and set one fixed delivery fee for each order.",
      shippingSearchPlaceholder: "Search governorates...",
      governorateName: "Governorate",
      shippingFeeLabel: "Delivery Fee",
      shippingAvailability: "Availability",
      shippingEnabled: "Enabled",
      shippingDisabled: "Disabled",
      shippingNoResults: "No governorates found for this search.",
      walletNumber: "Vodafone/Orange Cash Number",
    instapayHandle: "InstaPay Address (IPA)",
    walletPlaceholder: "e.g. 01012345678",
    instapayPlaceholder: "e.g. store{'@'}instapay",
    socialLabelFacebook: "Facebook",
    socialLabelInstagram: "Instagram",
    socialLabelX: "X (Twitter)",
    socialLabelWhatsapp: "WhatsApp",
    socialLabelTelegram: "Telegram",
    socialLabelGmail: "Gmail",
  },

  // ── Errors ────────────────────────────────────────────────
  errors: {
    required: "This field is required.",
    invalidEmail: "Please enter a valid email.",
    minLength: "Must be at least {min} characters.",
    notFound: "Page not found.",
    unauthorized: "You must be logged in to access this page.",
    forbidden: "You don't have permission to access this page.",
    serverError: "Server error. Please try again later.",
    invalidFileType: "Invalid file type. Please upload an image.",
  },

  // ── Profile ───────────────────────────────────────────
  profile: {
    title: "My Profile",
    subtitle: "Manage your personal information and password.",
    personalInfo: "Personal Information",
    fullName: "Full Name",
    phone: "Phone Number",
    username: "Full Name",
    usernamePlaceholder: "Enter your full name",
    email: "Email Address",
    emailPlaceholder: "Enter your email",
    changePassword: "Change Password",
    passwordHint: "Leave blank to keep your current password.",
    newPassword: "New Password",
    passwordPlaceholder: "At least 8 characters",
    confirmPassword: "Confirm Password",
    confirmPasswordPlaceholder: "Repeat new password",
    saveSuccess: "Profile updated successfully!",
    avatarSuccess: "Profile picture updated!",
    uploadAvatar: "Change profile picture",
    clickToChange: "Click the photo to change it",
    errors: {
      usernameRequired: "Name is required.",
      emailRequired: "Email is required.",
      emailInvalid: "Please enter a valid email address.",
      passwordTooShort: "Password must be at least 8 characters.",
      passwordMismatch: "Passwords do not match.",
      fileTooLarge: "File too large. Maximum size is 5 MB.",
    },
    pwd: {
      weak: "Weak",
      fair: "Fair",
      strong: "Strong",
      veryStrong: "Very strong",
    },
  },
  // ── Contact ──────────────────────────────────────────────
  contact: {
    title: "Contact Us",
    subtitle: "We would love to hear from you. Please fill out the form below and we will get back to you as soon as possible.",
    form: {
      name: "Name",
      email: "Email",
      subject: "Subject",
      message: "Message",
      namePlaceholder: "Your Name",
      emailPlaceholder: "your{'@'}email.com",
      subjectPlaceholder: "How can we help you?",
      messagePlaceholder: "Write your message here...",
      send: "Send Message",
      sending: "Sending...",
      success: "Message sent successfully!",
    },
    getInfo: "Get in Touch",
    social: {
      facebook: "Follow our page",
      instagram: "Follow us on Instagram",
      twitter: "Latest updates",
      whatsapp: "Start a Conversation",
      email: "Send us an email",
      telegram: "Message Us",
    },
    socialLabels: {
      facebook: "Facebook",
      instagram: "Instagram",
      twitter: "X / Twitter",
      whatsapp: "WhatsApp",
      email: "Email",
      telegram: "Telegram",
    },
  },

  // ── Error / Maintenance ───────────────────────────────────
  error: {
    maintenanceTitle: "We'll be right back!",
    maintenanceDesc:
      "Our server is taking a short break. Please try again in a few minutes.",
    retryBtn: "Try Again",
    statusCode: "Status",
  },
};
